...
export default async function handleWebhook(
  ...
  if (token === auth) {
    ...
    const shouldUpdateSectionItem =
      model === 'section_item' && event === 'entry.update';

    const shouldUpdateHomepage =
      shouldUpdateSection || shouldUpdateBlogs || shouldUpdateSectionItem;
