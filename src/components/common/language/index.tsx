...
const Language: React.FC<IProps> = ({className}) => {
  ...
  const targetLanguage = MODULES.language.items.filter(
    e => e.locale !== router.locale
  );
  ...
  const redirect = (path: string, locale: string) => {
    ...
    if (path.includes(ROUTES.CONTACT)) url = ROUTES.CONTACT;
      ...
      <>
        {targetLanguage.map((language: ILanguage, index) => {
          ...
          return (
            <span
              className="text"
              onClick={() => redirect(router.asPath, locale)}
              key={index}
            >
              {language.name}
            </span>
